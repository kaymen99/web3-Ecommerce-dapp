...
function Home() {
        ...
        if (openAuctions !== undefined) {
            ...
            const items = await Promise.all(
                openAuctions.map(async (auction) => {
                    let itemMetaData = await axios.get(auction[2])
                    let item = {
                        auctionId: Number(auction[0]),
                        name: itemMetaData.data.name,
                        image: itemMetaData.data.image,
    ...
    async function loadProducts() {
        ...
        const _allProducts = _marketProducts.concat(_allStoresProducts).sort(function (a, b) { return b.date - a.date });
        ...
        if (marketStores !== undefined) {
            const allStores = await Promise.all(
                marketStores.map(async (store) => {
                    ...
                    const meta = await axios.get(storeDetailsURL)
                    let item = {
                                ...
                                <Container>
                                    <Row style={{ display: "flex" }}>
                                        <Col className='col-md-6'>
                                            <h4 className='text-left p-2'>
                                                Main Market
                                            </h4>
                                        </Col>
                                        <Col className='col-md-6' style={{ display: "inline", textAlign: "right" }} >
                                            <h4 >
                                                <a className="btn btn-primary" href="/main-market">See All </a>
                                            </h4>
                                        </Col>
                                        <hr />
                                    </Row>
                                                            ...
                                                            <Card.Body>
                                                                ...
                                                                {product.store !== undefined ? (
                                                                    <a className="btn btn-primary"
                                                                        href={"/store-product/" + product.store + "/" + product.productId} role="button">See More</a>
                                                                ) : (
                                                                    <a className="btn btn-primary" href={"/products/" + product.productId} role="button">See More</a>
                                                                )}
                                                            </Card.Body>
                                                        </Card>
                                                    </Col>
                                    ...
                                    <Row style={{ display: "flex" }}>
                                        ...
                                        <Col className='col-md-6' style={{ display: "inline", textAlign: "right" }}>
                                            <h4 className='text-right p-2' >
                                                <a className="btn btn-primary" href="/all-stores">See All </a>
                                            </h4>
                                        </Col>
                                        <hr />
                                    </Row>
                                    <Row className='mt-5'>
                                        {
                                            allStores.map((store, id) => {
                                                ...
                                                return (
                                                    <Col style={{ marginBottom: "40px" }} md={3} key={id}>
                                                        <Card style={{ width: '16rem' }} key={id}>
                                                            <Card.Img variant="top" src={store.image} width="0px" height="180px" />
                                                            <Card.Body>
                                                                <Card.Title style={{ fontSize: "18px", textAlign: "center" }}>
                                                                    <a style={{ textDecoration: "none", color: "black" }} href={"/store/" + store.address}>{store.name}</a>
                                                                </Card.Title>
                                                            </Card.Body>
                                                        </Card>
                                                    </Col>
                            ...
                            {auctions.length !== 0 ? (
                                <Container>
                                    <Row style={{ display: "flex" }}>
                                        ...
                                        <Col className='col-md-6' style={{ display: "inline", textAlign: "right" }}>
                                            <h4 className='text-right p-2'>
                                                <a className="btn btn-primary" href="/auction-market">See All </a>
                                            </h4>
                                        </Col>
                                        <hr />
                                    ...
                                    <Row className='mt-5'>
                                        {auctions.map((auction, id) => {
                                            return (
                                                <Col style={{ marginBottom: "40px" }} md={3} key={id}>
                                                    <Card style={{ width: '16rem' }} key={id}>
                                                        <Card.Img variant="top" src={auction.image} width="0px" height="180px" />
                                                        <Card.Body>
                                                            <Card.Title style={{ fontSize: "14px" }}>
                                                                {auction.name}
                                                            </Card.Title>
                                                            ...
                                                            <a className="btn btn-primary" href={"/auctions/" + auction.auctionId} role="button">See More</a>
                                                        </Card.Body>
                                                    </Card>
                                                </Col>
                                            )
