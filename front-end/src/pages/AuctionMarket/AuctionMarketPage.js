...
function AuctionMarketPage() {
    ...
    async function loadAuctions() {
        ...
        const openAuctions = allAuctions.filter(p => p[7] === auctionStatusMap["OPEN"])
        if (openAuctions !== undefined) {
            const items = await Promise.all(
                openAuctions.map(async (auction) => {
                    let metaData = await axios.get(auction[2])
                    let item = {
    ...
    function findAuction() {
        if (search !== "") {
            setLoading(true)
            const foundAuctions = auctions.filter(p => p.name.toLowerCase().includes(search))
                    ...
                    isGoodNet ? (
                        <>
                            <Form className="d-flex">
                                ...
                                />
                                <Button variant="outline-info" onClick={() => { findAuction() }}>
