...
function AuctionPage() {
    ...
    const auctionDetails = async (auctionId) => {
        if (id !== undefined) {
            const market = new ethers.Contract(auctionContractAddress, AuctionContract.abi, provider);
            ...
            let metaData = await axios.get(details[2])
    ...
    const bid = async (auctionId) => {
        try {
            if (auctionId !== undefined) {
                ...
                const signer = provider.getSigner()
                const market = new ethers.Contract(auctionContractAddress, AuctionContract.abi, signer);

                const bid_tx = await market.bid(auctionId, { value: utils.parseEther(bid_in_eth.toString(), "ether") })
                await bid_tx.wait()
    ...
    async function convertPrice(amount) {
        const market = new ethers.Contract(auctionContractAddress, AuctionContract.abi, provider);
        const price_in_eth = await market.callStatic._convertUSDToETH(utils.parseEther(amount, "ether"))
        return price_in_eth;
    }
                ...
                <div className='col-md-7 text-center p-3'>
                    <div className='p-3'>
                        {auctionState.status === "OPEN" ? (
                            <div>
                                Sale of <b>{auctionState.name}</b> for <b>{parseFloat(auctionState.highest_bid_in_ETH).toFixed(4)} ETH</b>
                            </div>
                        ) : (
                            <div>
                                <b>{auctionState.name}</b> bought for <b>{parseFloat(auctionState.highest_bid_in_ETH).toFixed(4)} ETH</b>
                            </div>
                        )}

                        <div>{auctionState.description}</div>
                        <br />
                        <img className='rounded' src={auctionState.image} height="350px" width="560px" />
                        <br />
                        <br />
                            ...
                            auctionState.status === "OPEN" ? (
                                moment.unix(auctionState.endTime).isBefore(Date.now()) ? (
                                    <Button
                                        ...
                                        {loading ? <CircularProgress size={26} color="#fff" /> : "Close"}
                                    </Button>
                                ) : (
                                        ...
                                        <>
                                            <div className='col-md-4' style={{ marginLeft: "35%" }}>
                                                ...
                                                <Button
                                                    variant='primary'
                                                    onClick={() => { bid(id) }}>
                                                    {loading ? <CircularProgress size={26} color="#fff" /> : "OutBid"}
                                                </Button>
                                                <Button
                                                    style={{ marginLeft: "5px" }}
                                                    variant='danger'
                                                    onClick={() => { withdraw(id) }}>
                                                    {loading ? <CircularProgress size={26} color="#fff" /> : "Withdraw"}
                                                </Button>
                                            </div>
                                        </>
                                    ) : (
                                    ...
                                    <>
                                        <div className='col-md-3' style={{ marginLeft: "35%" }}>
                                            ...
                                            <Button
                                                variant='primary'
                                                onClick={() => { bid(id) }}>
                                                {loading ? <CircularProgress size={26} color="#fff" /> : "Bid"}
                                            </Button>
                                        </div>
                                    </>
                                ) : (
                    ...
                    <Table responsive>
                        <tbody>
                            <tr>
                                ...
                                <td>{data.account === auctionState.seller ? "you are the seller" : auctionState.seller}</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>{moment.unix(auctionState.endTime).format('MMM D, YYYY, HH:mmA')}</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>{parseFloat(auctionState.highest_bid_in_ETH).toFixed(5)} ETH</td>
                            </tr>
                            <tr>
                                <td className='p-2'>Highest Bidder</td>
                                <td>
                                    {data.account === auctionState.highestBidder ?
                                        "You are the highest Bidder" : auctionState.highestBidder === ethers.constants.AddressZero ?
                                            "No Bidder yet" : auctionState.highestBidder
                                    }
                                </td>
                            </tr>
                            {data.account !== auctionState.seller ? (
