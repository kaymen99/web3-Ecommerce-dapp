...
function OrderPage() {
    ...
    const orderDetails = async () => {
        if (Number(order_id) !== undefined) {
            const productStore = new ethers.Contract(store, StoreContract.abi, provider);
            const storeOwner = await productStore.callStatic.owner()

            const product_orders = await productStore.listStoreOrders()

            const order = product_orders.filter(o => Number(o[0]) === Number(order_id))[0]
    ...
    const fillOrder = async () => {
        try {
            if (Number(order_id) !== undefined) {
                setLoading(true)
                const signer = provider.getSigner()
                const productStore = new ethers.Contract(store, StoreContract.abi, signer);
    ...
    const confirm = async () => {
        try {
            if (Number(product_id) !== undefined) {
                ...
                const confirm_tx = await productStore.confirmRecieved(
                    Number(order_id)
                )
    ...
    const cancel = async () => {
        try {
            if (Number(order_id) !== undefined) {
                ...
                const cancel_tx = await productStore.cancelOrder(
                    Number(order_id)
                )
        ...
        <>
            <div className='row p-2'>
                <div className='col-md-7 text-center p-3'>
                    <div className='p-3'>
                        <div>Order of <b id='itemname'>{orderState.name}</b> for <b id='itemprice'>{parseFloat(orderState.total_buy_price).toFixed(4)} ETH</b></div>
                        <br />
                        <img className='rounded' src={orderState.image} height="350px" width="560px" />
                        <br />
                        <br />
                    ...
                    <Table responsive>
                        <tbody>
                            <tr>
                                ...
                                <td>{data.account === orderState.seller ? "you are the seller" : orderState.seller}</td>
                            </tr>
                            <tr>
                                <td className='p-2'>Buyer</td>
                                <td>
                                    {data.account === orderState.buyer ?
                                        "You are the buyer" : orderState.buyer
                                    }
                                </td>
                            </tr>
                            <tr>
                                        ...
                                        <Modal.Body>
                                            ...
                                            <div>
                                                ...
                                                <Form.Control as="textarea"
                                                    ...
                                                    required
                                                    onChange={(e) => { setFormInput({ ...formInput, review: e.target.value }) }} />
                                            </div>
                                            <br />
                                        </Modal.Body>
                                        <Modal.Footer>
                                            ...
                                            <Button variant="primary"
                                                type='submit'
                                                onClick={() => { review_product() }}>
                                                {loading ? <CircularProgress size={26} color="#fff" /> : "Submit"}
                                            </Button>
                                        </Modal.Footer>
                                    </Modal>
                                </>
