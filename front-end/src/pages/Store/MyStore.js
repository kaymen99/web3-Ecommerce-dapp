...
function MyStore() {
    ...
    const [formInput, setFormInput] = useState({ name: "", image: null })
    const [imagePreview, setImagePreview] = useState(null)

    const [show, setShow] = useState(false);
    const handleClose = () => {
        setShow(false)
        setFormInput({ name: "", image: null })
    ...
    const getStore = async () => {
        ...
        const factory = new ethers.Contract(factoryAddress, StoreFactoryContract.abi, provider);

        const marketStores = await factory.getAllStores();

        const myStore = marketStores.filter(storeInfo => storeInfo[1] === data.account)

        if (myStore.length !== 0) {

            dispatch(
                getUserData({
                    ...userMarketItems, store: myStore[0][0]
                })
            )
            setHasStore(true)
        }
    ...
    const createStore = async () => {
        ...
        if (!hasStore) {
            try {
                ...
                const addedFile = await ipfsClient.add(formInput.image)
                const imageURI = ipfsBaseUrl + addedFile.path

                const data = JSON.stringify({
                    name: formInput.name, image: imageURI
                })
                const addedData = await ipfsClient.add(data)
                const descriptionURI = ipfsBaseUrl + addedData.path

                const create_store_tx = await factory.createStore(descriptionURI, { value: create_store_fee })
    ...
    async function remove(id) {
        ...
        const Store = new ethers.Contract(myStoreAddress, StoreContract.abi, signer);
    ...
    async function loadMyProducts() {
        if (userMarketItems.store !== "") {
            const signer = provider.getSigner()
            const myStoreAddress = userMarketItems.store
            const productStore = new ethers.Contract(myStoreAddress, StoreContract.abi, signer);
    ...
    const getImage = async (e) => {
        ...
        if (file !== undefined) {
            ...
            reader.onloadend = () => {
                ...
                console.log(buf)
                setFormInput({ ...formInput, image: buf })
    ...
    const loadMyOrders = async () => {
        if (userMarketItems.store !== "") {
            ...
            const uncomplete_orders = orders.filter(o => orderStatus[o[6]] !== "COMPLETED")

            if (uncomplete_orders !== undefined) {
                const items = await Promise.all(uncomplete_orders.map(async (order) => {
                        ...
                        hasStore ? (
                                            ...
                                            <div className={classes.Container}>
                                                <p>
                                                    ...
                                                    <a className="btn btn-primary"
                                                        style={{ margin: "4px" }}
                                                        href={"/add-product/" + userMarketItems.store} role="button">
                                                        Add Product
                                                    </a>
                                                </p>
                                                            ...
                                                            <Col md={3} style={{ marginBottom: "40px" }} key={id}>
                                                                <Card style={{ width: '16rem' }} key={id}>
                                                                    ...
                                                                    <Card.Body>
                                                                        ...
                                                                        <a className="btn btn-primary"
                                                                            style={{ margin: "4px" }}
                                                                            href={"/store-product/" + userMarketItems.store + "/" + product.productId} role="button">
                                                                            See More
                                                                        </a>
                                                        ...
                                                        {storeOrders.map((order, index) => {
                                                            return (
                                                                <tr key={index}>
                                                                    <td>
                                                                        <a
                                                                            href={"/order/" + userMarketItems.store + "/" + order.productId + "/" + order.orderId}>
                                                                            {order.orderId}
                                                                        </a>
                                                                    ...
                                                                    <td>{parseFloat(order.TotalbuyPrice).toFixed(5)}</td>
                                                                    <td>{order.status}</td>

                                                                    {order.status === "PENDING" ? (
                                                                        <td key={index}>
                                                                            <Button variant='primary'
                                                                                onClick={() => { fillOrder(order.orderId) }} >
                                                                                {loading ? <CircularProgress size={26} color="#fff" /> : "Fill"}
                                                                            </Button>
                                                                        </td>
                                                                    ) : null
                                                                    }
                            ...
                            <div className={classes.Container}>
                                ...
                                <Button variant='primary' onClick={handleShow}>
                                    {loading ? <CircularProgress size={26} color="#fff" /> : "Create"}
                                </Button>
