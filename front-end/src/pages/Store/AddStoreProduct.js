...
function AddProduct() {
    ...
    const [formInput, setFormInput] = useState({ name: "", description: "", price: 0, quantity: 0, type: 1 })
    ...
    const addProduct = async () => {
        if (image !== undefined) {
            try {
                ...
                setLoading(false)
                setImage(null)
                setFormInput({ name: "", description: "", price: 0, quantity: 0, type: 0 })
                    ...
                    isGoodNet ? (
                        <>
                            ...
                            <Form.Group className="mb-3">
                                ...
                                <Form.Control type="text" maxLength={30} placeholder="Enter product name" onChange={(e) => { setFormInput({ ...formInput, name: e.target.value }) }} required />
                            </Form.Group>

                            <br />
                            <div>
                                <label>Product Description: </label>
                                <Form.Control as="textarea" rows={5} maxLength={200} placeholder="Enter product description" onChange={(e) => { setFormInput({ ...formInput, description: e.target.value }) }} required />
                            </div>
                            <br />
                            <div>
                                <label>Product price: </label>
                                <Form.Control type="number" step="any" min="0" placeholder="Enter product price in $" onChange={e => setFormInput({ ...formInput, price: e.target.value })} required />
                            </div>
                            <br />
                            <div>
                                ...
                                <div key={`inline-radio`} className="mb-3">
                                    <Form.Check
                                        ...
                                        value={0}
                                        id={`inline-radio-1`}
                                        onClick={e => setFormInput({ ...formInput, type: Number(e.target.value) })}
                                        required
                                    />
                                    <Form.Check
                                        inline
                            ...
                            {formInput.type === 0 ? (
                                <div>
                                    <Form.Control type="number" min="1" placeholder="Enter product quantity" onChange={e => setFormInput({ ...formInput, quantity: e.target.value })} required />
                                </div>
                            ) : null}
                            <br />
                            <div >
