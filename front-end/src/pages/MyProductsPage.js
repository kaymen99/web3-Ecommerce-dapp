            ...
            allStores.map(async (store) => {
                ...
                await Promise.all(
                    my_orders.map(async (order) => {
                        const product = await productStore.callStatic.storeProducts(Number(order[1]))
                        let item = {
                                            ...
                                            {allMyOrders.length !== 0 ? (
                                                allMyOrders.map((order, id) => {
                                                    return (
                                                        <Col md={3} style={{ margin: "20px" }} key={id}>
                                                            <Card style={{ width: '16rem' }} key={id}>
                                                                <Card.Img variant="top" src={order.image} width="0px" height="180px" />
                                                                <Card.Body>
                                                                    <Card.Title style={{ fontSize: "14px" }}>
                                                                        {order.name}
                                                                    </Card.Title>
                                                                    ...
                                                                    <div style={{ display: 'inline' }}>
                                                                        <a className="btn btn-primary"
                                                                            style={{ margin: "4px" }}
                                                                            href={"/order/" + order.store + "/" + order.productId + "/" + order.orderId} role="button">
                                                                            See More
                                                                        </a>
                                                                    </div>
